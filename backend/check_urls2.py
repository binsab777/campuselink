...
def fully_clean(s):
    # remove anything that looks like a variable placeholder or number
    s = re.sub(r'\{[^}]*\}', '', s)
...
for file in glob.glob('../backend-laravel/tests/**/*.php', recursive=True):
    with open(file, 'r', encoding='utf-8', errors='ignore') as f:
        ...
        # also find concat like '/api/v1/skills/' . $skillId
...
print(f"Verified: {verified}, Not Verified: {not_verified}")
